...
class Remember_me {
  ...
  // END Remember_me
    ...
    */
    function set()
    {
      ...
      foreach ($this->_entry_id as $entry_id)
      {
        if ($entry = $this->_entry_exists($entry_id))
        {
          $this->_storage[$entry_id] = $entry;
        }
      }
    ...
    */
    ...
    if ($this->_entry_id)
    {
      foreach ($this->_entry_id as $entry_id)
      {
        $entry = $this->_entry_exists($entry_id);
  ...
  */
  ...
  function _remove_entry_from_list()
  {
    ...
    foreach ($this->_entry_id as $entry_id)
    {
      if (isset($lists[$this->_list][$entry_id]))
      {
        unset($lists[$this->_list][$entry_id]);
      }
    }
  ...
  */
  ...
  function _fetch_all_params()
  {
    ...
    $this->_entry_id = $this->_fetch_param('entry_id') ? array_filter( explode('|', $this->_fetch_param('entry_id') ) ) : FALSE;
